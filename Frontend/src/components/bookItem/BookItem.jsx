...
const BookItem = ({ lecture, onUpdate, onDelete }) => {
  ...
  const authorName = book?.authorName || "Desconocido";
  const authorId = book?.authorId || null;

  // Valores iniciales, normalizados para evitar uncontrolled -> controlled
  const [editStatus, setStatus] = useState(status || "");
  const [editRating, setRating] = useState(rating !== undefined && rating !== null ? rating : "");
  const [editPageCount, setPageCount] = useState(pageCount !== undefined && pageCount !== null ? pageCount : 0);
  ...
  const handleEditStartedDate = (event) => setEditStarted(event.target.value);
  const handleEditFinishedDate = (event) => setEditFinished(event.target.value);
  ...
  const handleEditPageCount = (event) => {
    const value = event.target.value;
    setPageCount(value !== "" ? parseInt(value) : 0);
  }
            ...
            {isEditing ?
              <>
                <FormControl
                  ...
                  style={{ width: '50px' }}
                  step="1"
                  value={editPageCount !== undefined ? editPageCount : 0}
              ...
              :
              (status === 'Leído' || status === 'Para leer') ? pages : `${pageCount} / ${pages}`
            }
          </FormGroup>
        </Col>
